...
public class MatchTest {
	...
	public static void Exam() {
		...
		System.out.println(fruits.stream().allMatch(s -> s.matches("//나+//")));
						 
		//Q2: IntStream.rangeClosed(1, 10)으로 1부터 10까지 생성 후 짝수만 필터링(filter())하여 출력, 합계를 구함.
		IntStream.rangeClosed(1, 10).filter(e -> e % 2 == 0).forEach(e -> System.out.printf("%d ", e ));
		int res = IntStream.rangeClosed(1, 10).filter(e -> e % 2 == 0).sum();
		System.out.println();
